...
public class Punctuation 
{
    ...
    {
    	String passage = new String("Blood, Sweat, and Tears by Winston Churchill "+
    			"May 13, 1940 "+
    			"Mr. Speaker: "+
    			...
    			"govetaking in itself, but it must be remembered that we are in the preliminary stage of "+
    			"one of the greatest battles in history, that we are in action at many points in Norway and "+
    			"in Hrnment: \"I have nothing to offer but blood, toil, tears and sweat.\" "+
        ...
        String punctuationWords = getPunctuationWords(passage);
        printWords(punctuationWords);
    }

    public String getPunctuationWords(String passageIn) 
    {
        String result = new String(""); // To store words with punctuation
        String word = new String("");   // Temporary storage for the current word

        for (int i = 0; i < passageIn.length(); i++) 
        {
        ...
        // Check the last word in the passage (if any)
        if (checkForPunctuation(word)) 
        {
            result += word + " ";
        }
    ...
    public boolean checkForPunctuation(String word) 
    {
        ...
        for (int i = 0; i < word.length(); i++) 
        {
            ...
            {
                return true; // Return true if a punctuation mark is found
            }
        }
        return false; // No punctuation found
    }

    public void printWords(String toPrintIn) 
    {
